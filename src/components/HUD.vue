<template>
	<div class="hud-container">
		...
		<div class="hud-grid">
			...
			<button class="hud-slot hud-slot-active" @click="openBookModal" data-tooltip="Información" aria-label="Abrir información">
			...
			<!-- Slot 2: Historial -->
			<button class="hud-slot hud-slot-active" @click="openHistory" data-tooltip="Historial" aria-label="Abrir historial">
				<div class="hud-slot-inner">
					<img src="/icons/Hanging_Sign.webp" alt="Historial" class="hud-slot-icon" />
				</div>
			</button>

			<!-- Slot 3: Panel de Notificaciones -->
			<button class="hud-slot hud-slot-active" @click="openNotifications" data-tooltip="Notificaciones" aria-label="Abrir notificaciones">
				<div class="hud-slot-inner">
					<img src="/icons/icon_bell.webp" alt="Notificaciones" class="hud-slot-icon" />
				</div>
			</button>
			...
			<button class="hud-slot hud-slot-active hud-slot-help" @click="openHelpModal" data-tooltip="¡Pedir ayuda!" aria-label="Pedir ayuda">
				<div class="hud-slot-inner help-slot">
					<img src="/icons/abcde.gif" alt="Pedir ayuda" class="hud-slot-icon help-icon" />
				</div>
			</button>
		</div>
...
<style scoped>
	...
	.hud-grid {
		...
		overflow: visible;
		background: linear-gradient(180deg, rgba(24, 30, 24, 0.6) 0%, rgba(0, 0, 0, 0.15) 100%);
	...
	.hud-grid::before {
		...
		background:
    /* scanlines */
			repeating-linear-gradient(0deg, rgba(0, 0, 0, 0.12), rgba(0, 0, 0, 0.12) 2px, transparent 2px, transparent 4px),
			/* subtle block grid */ repeating-linear-gradient(90deg, rgba(255, 255, 255, 0.03) 0px, rgba(255, 255, 255, 0.03) 2px, transparent 2px, transparent 20px);
		pointer-events: none;
		opacity: 0.55;
		border-radius: 12px;
		z-index: 0;
    ...
    /* pixel noise */
			...
			/* subtle "block" grid */ repeating-linear-gradient(90deg, rgba(255, 255, 255, 0.05) 0px, rgba(255, 255, 255, 0.05) 2px, transparent 2px, transparent 18px),
			repeating-linear-gradient(0deg, rgba(0, 0, 0, 0.12) 0px, rgba(0, 0, 0, 0.12) 2px, transparent 2px, transparent 18px);
		pointer-events: none;
	}
