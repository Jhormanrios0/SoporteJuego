-- Adds an optional admin note to each life event.
-- This allows profiles to show both the predefined/random death reason and the admin-provided message.

ALTER TABLE public.life_events
  ADD COLUMN IF NOT EXISTS admin_message TEXT;

-- Replace function to store admin_message separately (optional input from admin)
-- and keep the predefined/random death message in `reason`.
...
BEGIN
  ...
  admin_note := NULLIF(BTRIM(p_reason), '');
  ...
  INSERT INTO life_events (player_id, admin_id, delta, reason, admin_message)
  VALUES (p_player_id, auth.uid(), -p_amount, death_message, admin_note);
END;
$$;

-- Keep historical rows consistent (optional): normalize empty strings to NULL
